...
```bash
curl.exe -X POST "http://127.0.0.1:8011/v1/finetune/jobs" \
...
```bash
curl.exe "http://127.0.0.1:8011/v1/finetune/jobs/<JOB_ID>"
```

## 3. 发布模型

```bash
curl.exe -X POST "http://127.0.0.1:8011/api/model/publish" \
  -H "Content-Type: application/json" \
...
```bash
curl.exe -X POST "http://127.0.0.1:8011/api/model/infer" \
  -H "Content-Type: application/json" \
...
```bash
curl.exe "http://127.0.0.1:8011/api/model/infer/config?model_path=<MODEL_PATH>"
```

## 6. 分段推理（首段）

```bash
curl.exe -X POST "http://127.0.0.1:8011/api/model/infer/chunk" \
  -H "Content-Type: application/json" \
